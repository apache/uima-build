...
for i in target/eclipse-update-site/*.jar; do gpg --output $i.asc --detach-sig --armor $i; done

# Create MD5 checksums
for i in target/eclipse-update-site/*.jar; do md5sum --binary $i > $i.md5; done

# Create SHA1 checksums
for i in target/eclipse-update-site/*.jar; do sha1sum --binary $i > $i.sha1; done
